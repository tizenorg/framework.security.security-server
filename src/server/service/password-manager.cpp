...
namespace SecurityServer
{
                                       ...
                                       unsigned int &maxAttempt, unsigned int &expirationTime)
    {
        ...
        bool isExpired = m_pwdFile.checkExpiration();
        bool areAttemptsExceeded = m_pwdFile.checkIfAttemptsExceeded();

        if (isExpired && areAttemptsExceeded) {
            LogError("Password expired; max attempts exceeded.");
