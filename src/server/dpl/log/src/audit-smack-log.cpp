...
int AuditSmackLog::ProcessLogDir()
{
    ...
    size_t len = offsetof(struct dirent, d_name) + pathconf(AUDIT_LOG_DIRECTORY.c_str(), _PC_NAME_MAX) + 1;
    dirent *dentry = static_cast<struct dirent *>(malloc(len));
    ...
    while (!readdir_r(dir, dentry, &dp) && dp) {
