...
{
    if(quota_max)
        *quota_max = SecurityServer::SHARED_FILE_QUOTA;
    if(num_files_max)
        *num_files_max = SecurityServer::SHARED_FILE_MAX_NUM;
}
...
int security_server_open_for(const char *filename, int *fd)
{
    ...
    return try_catch([&] {
        ...
        int result = sendToServerAncData(SERVICE_SOCKET_OPEN_FOR_UNPRIVILEGED, send.Pop(), hdr);
...
int security_server_shared_file_reopen(const char *filename, int *fd)
{
    ...
    return try_catch([&] {
        ...
        int result = sendToServerAncData(SERVICE_SOCKET_OPEN_FOR_PRIVILEGED, send.Pop(), hdr);
        if (result != SECURITY_SERVER_API_SUCCESS) {
            *fd = -1;
            return result;
        }
