...
static int rdb_end(sqlite3 *p_db, int i_session_ret)
{
	...
	if(i_session_ret == PC_OPERATION_SUCCESS && !b_shared_transaction__) {
		...
		if(have_smack()) {
			...
			ret = dump_smack_rules(p_db);
			if(ret != PC_OPERATION_SUCCESS) {
				C_LOGE("RDB: Error dumping smack rules: %d", ret);
				goto finish;
			}
		}
	}
			       ...
			       const bool   b_is_volatile)
{
	...
	const char *const *tmp = pp_permissions_list;
	while (*tmp != NULL) {
		C_LOGD("\tpermission: %s\n", *tmp);
		tmp++;
	}
		    ...
		    const char *const s_path)
{
	...
	ret = add_sharing_internal(p_db, s_owner_label_name, s_target_label_name, s_path);
...
int rdb_get_pair_sharing_count(const char *const s_owner_label_name,
			       const char *const s_target_label_name,
			       int *p_sharing_count)
{
	...
	ret = get_pair_sharing_count_internal(p_db, s_owner_label_name, s_target_label_name,
					      p_sharing_count);
finish:
	return rdb_finish(p_db, ret);
}
...
int rdb_get_owner_sharing_count(const char *const s_owner_label_name, int *p_sharing_count)
{
	...
	ret = get_owner_sharing_count_internal(p_db, s_owner_label_name, p_sharing_count);
...
int rdb_get_path_sharing_count(const char *const s_path, int *p_sharing_count)
{
	...
	ret = get_path_sharing_count_internal(p_db, s_path, p_sharing_count);
			  ...
			  int *p_sharing_count)
{
	...
	ret = get_sharing_count_internal(p_db, s_owner_label_name, s_target_label_name, s_path,
					 p_sharing_count);
		       ...
		       const char *const s_path)
{
	...
	ret = remove_sharing_internal(p_db, s_owner_label_name, s_target_label_name, s_path);
...
int rdb_process_sharing(process_sharing_func func)
{
	...
	ret = process_sharing_internal(p_db, func);
...
int rdb_clear_sharing(void)
{
	...
	ret = clear_sharing_internal(p_db);
