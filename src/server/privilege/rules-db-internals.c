...
#ifndef _GNU_SOURCE     // For asprintf and TEMP_FAILURE_RETRY
#define _GNU_SOURCE
#endif
...
#include <unistd.h>     // For sleep and TEMP_FAILURE_RETRY
#include <sys/types.h>  // For rule file access
#include <sys/stat.h>   // For rule file access
#include <fcntl.h>      // For rule file access
...
static const char* const DB_DIR = "/opt/dbspace/";
static const char* const RULE_FILE = "/opt/dbspace/.rules_file";
static const char* const RULE_FILE_TMP = "/opt/dbspace/.rules_file.tmp";
	...
	for(i = 0; pp_smack_rules[i] != NULL ; ++i) {
		...
		if(!strcmp(ps_object, "~PUBLIC_PATH~")) {
			...
			ret = add_label_app_path_type_rule(p_label_to_app_path_type_stmt,
							   ...
							   is_reverse);
			if(ret != PC_OPERATION_SUCCESS) goto finish;
		} else if(!strcmp(ps_object, "~SHARED_PATH~")) {
			ret = add_label_app_path_type_rule(p_label_to_app_path_type_stmt,
							   ps_subject,
							   "SHARED_PATH",
							   s_access,
 ...
 * Sync parent dir
 *
 * It is unclear if and how it should be done:
 * 1. The fsync() manual says that parent directory has to be explicitly flushed after file
 *    has been created/renamed.
 * 2. It also says that fsync() can only accept a descriptor open for writing.
 * 3. RW descriptor can't be obtained for a directory.
 * 4. For ext4 in current kernel fsync() doesn't check the mode for which the descriptor has
 *    been opened.
 * 5. It also synchronizes parent directory of newly created file.
 */
static int sync_db_dir()
{
	int ret = PC_OPERATION_SUCCESS;

	int dirfd = open(DB_DIR, O_RDONLY);
	if (dirfd < 0) {
		C_LOGE("Cannot open db dir %s for reading: %s", DB_DIR, strerror(errno));
		return PC_ERR_FILE_OPERATION;
	}

	if (0 != fsync(dirfd)) {
		C_LOGE("fsync() failed: %s", strerror(errno));
...
static int write_rules(sqlite3 *p_db, int fd)
{
	RDB_LOG_ENTRY;

	sqlite3_stmt *p_stmt = NULL;
	int ret = prepare_stmt(p_db, &p_stmt,
			"SELECT subject || ' ' || object || ' ' || access_to_str(bitwise_or(access)) || ' -' || x'0a'\
			 FROM   all_smack_binary_rules\
			 WHERE  is_volatile = 0\
			 GROUP  BY subject, object\
			 ORDER  BY subject, object ASC;");
	...
	while ((ret = sqlite3_step(p_stmt)) == SQLITE_ROW) {
		const char* rule = (const char *) sqlite3_column_text(p_stmt, RDB_FIRST_COLUMN);

		size_t len = strlen(rule);
		size_t written = 0;
		while(written < len) {
			ssize_t temp = TEMP_FAILURE_RETRY(write(fd, &rule[written], len-written));
			if(-1 == temp) {
				C_LOGE("Error in write: %s", strerror(errno));
				ret = PC_ERR_FILE_OPERATION;
				goto finish;
			}
			written += temp;
...
int dump_smack_rules(sqlite3 *p_db)
{
	...
	if (0 != unlink(RULE_FILE) && ENOENT != errno) {
		C_LOGE("Cannot remove the old rule file %s", RULE_FILE, strerror(errno));
		return PC_ERR_FILE_OPERATION;
	}

	int fd = TEMP_FAILURE_RETRY(creat(RULE_FILE_TMP, S_IWUSR | S_IRUSR));
	if (fd < 0) {
		C_LOGE("Cannot open rule file %s for writing: %s", RULE_FILE_TMP, strerror(errno)) ;
		ret = PC_ERR_FILE_OPERATION;
		goto finish;
	}

	ret = write_rules(p_db, fd);
	...
	if (ret == SQLITE_DONE) {
		...
		if (0 != rename(RULE_FILE_TMP, RULE_FILE)) {
			C_LOGE("Cannot rename %s to %s", RULE_FILE_TMP, RULE_FILE, strerror(errno));
			ret = PC_ERR_FILE_OPERATION;
			goto finish;
		}
	}
...
finish:
	...
	// always sync the directory
	int sync_ret = sync_db_dir();
	if (ret == PC_OPERATION_SUCCESS && sync_ret != PC_OPERATION_SUCCESS)
		ret = sync_ret;
		...
		perm_blacklist_status_t** pp_perm_list, size_t i_perm_number)
{
	...
	*pp_perm_list = (perm_blacklist_status_t*) malloc(
	...
	while ((ret = sqlite3_step(p_stmt)) == SQLITE_ROW && i < i_perm_number) {
		(*pp_perm_list)[i].permission_name = NULL;
		ret = perm_from_base_name(
			(const char *) sqlite3_column_text(p_stmt, RDB_FIRST_COLUMN),
			&((*pp_perm_list)[i].permission_name));
		if(ret != PC_OPERATION_SUCCESS) {
			C_LOGE("RDB: Failed to parse blacklist permission");
			continue;
		}
...
{
	RDB_LOG_ENTRY_PARAM("%s %s %s", s_owner_label_name, s_target_label_name, s_path);

	int ret = PC_ERR_DB_OPERATION;
	sqlite3_stmt *p_stmt = NULL;

	ret = prepare_stmt(p_db, &p_stmt,
			   "INSERT OR REPLACE INTO app_shared_path                \
			    VALUES(%Q, %Q, %Q,                                \
			          COALESCE((SELECT counter FROM app_shared_path   \
			                    WHERE owner_label_name=%Q        \
			                    AND target_label_name=%Q         \
			                    AND path=%Q), 0) + 1);",
			   s_owner_label_name, s_target_label_name, s_path,
			   s_owner_label_name, s_target_label_name, s_path);
	if(ret != PC_OPERATION_SUCCESS)
		goto finish;
...
int get_owner_sharing_count_internal(sqlite3 *p_db, const char *const s_owner_label_name,
				     int *p_sharing_count)
{
	RDB_LOG_ENTRY_PARAM("%s", s_owner_label_name);

	int ret = PC_ERR_DB_OPERATION;
	sqlite3_stmt *p_stmt = NULL;

	ret = prepare_stmt(p_db, &p_stmt,
			   "SELECT COALESCE(SUM(counter),0) FROM app_shared_path WHERE owner_label_name=%Q;",
			   s_owner_label_name);
	if (ret != PC_OPERATION_SUCCESS)
		goto finish;
	...
	ret = sqlite3_step(p_stmt);
	if (ret == SQLITE_ROW) {
		*p_sharing_count = sqlite3_column_int(p_stmt, RDB_FIRST_COLUMN);
		ret = PC_OPERATION_SUCCESS;
	} else {
		C_LOGE("RDB: Error during stepping: %s", sqlite3_errmsg(sqlite3_db_handle(p_stmt)));
		ret = PC_ERR_DB_QUERY_STEP;
	}
...
{
	RDB_LOG_ENTRY_PARAM("%s %s", s_owner_label_name, s_target_label_name);

	int ret = PC_ERR_DB_OPERATION;
	sqlite3_stmt *p_stmt = NULL;

	ret = prepare_stmt(p_db, &p_stmt,
			   "SELECT COALESCE(SUM(counter),0) FROM app_shared_path WHERE"
			   " owner_label_name=%Q AND target_label_name=%Q;",
			   s_owner_label_name, s_target_label_name);
	if (ret != PC_OPERATION_SUCCESS)
		goto finish;
...
{
	RDB_LOG_ENTRY_PARAM("%s", s_path);

	int ret = PC_ERR_DB_OPERATION;
	sqlite3_stmt *p_stmt = NULL;

	ret = prepare_stmt(p_db, &p_stmt,
			   "SELECT COALESCE(SUM(counter),0) FROM app_shared_path WHERE path=%Q;", s_path);
	if (ret != PC_OPERATION_SUCCESS)
		goto finish;
			       ...
			       int *p_sharing_count)
{
	...
	ret = prepare_stmt(p_db, &p_stmt,
			   "SELECT COALESCE(counter,0) FROM app_shared_path WHERE"
			   "  owner_label_name=%Q and target_label_name=%Q and path=%Q;",
			   s_owner_label_name, s_target_label_name, s_path);
	if (ret != PC_OPERATION_SUCCESS)
		goto finish;
			    ...
			    const char *const s_target_label_name, const char *const s_path)
{
	...
	switch(ret) {
	case SQLITE_ROW:
		counter = sqlite3_column_int(p_stmt, RDB_FIRST_COLUMN);
		break;
	default:
		C_LOGE("RDB: Error during stepping: %s", sqlite3_errmsg(sqlite3_db_handle(p_stmt)));
		ret = PC_ERR_DB_QUERY_STEP;
	...
	if (counter > 1) {
		ret = prepare_stmt(p_db, &p_stmt,
				   "UPDATE app_shared_path SET counter = counter - 1 WHERE"
				   "  owner_label_name=%Q and target_label_name=%Q and path=%Q;",
				   s_owner_label_name, s_target_label_name, s_path);
		if (ret != PC_OPERATION_SUCCESS)
			goto finish;
		...
		ret = prepare_stmt(p_db, &p_stmt,
				   "DELETE FROM app_shared_path WHERE"
				   "  owner_label_name=%Q and target_label_name=%Q and path=%Q;",
				   s_owner_label_name, s_target_label_name, s_path);
		if (ret != PC_OPERATION_SUCCESS)
			goto finish;
...
int process_sharing_internal(sqlite3 *p_db, process_sharing_func func)
{
	...
	sqlite3_stmt *p_stmt = NULL;
	int ret = prepare_stmt(p_db, &p_stmt,
			       "SELECT owner_label_name, target_label_name, path FROM app_shared_path "
			       "ORDER BY owner_label_name, target_label_name");
	if (ret != PC_OPERATION_SUCCESS )
			goto finish;

	while((ret = sqlite3_step(p_stmt)) == SQLITE_ROW) {
		ret = PC_ERR_DB_OPERATION;
		const char *owner = (const char *)sqlite3_column_text(p_stmt, RDB_FIRST_COLUMN);
		if (owner == NULL) {
			goto finish;
		}
		const char *target = (const char *)sqlite3_column_text(p_stmt, RDB_SECOND_COLUMN);
		if (target == NULL) {
			goto finish;
		}
		const char *path = (const char *)sqlite3_column_text(p_stmt, RDB_THIRD_COLUMN);
		if (path == NULL) {
			goto finish;
		}
		func(owner, target, path);
	}
...
int clear_sharing_internal(sqlite3 *p_db)
{
	...
	sqlite3_stmt *p_stmt = NULL;
	int ret = prepare_stmt(p_db, &p_stmt, "DELETE FROM app_shared_path;");
